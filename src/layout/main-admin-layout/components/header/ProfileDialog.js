...
export default function ProfileDialog({ is_open, onClose }) {
  const [is_loading_update_profile, setIsLoadingUpdateProfile] =
    useState(false);
  ...
  useEffect(() => {
    ...
    axios
      .get(`${Http.HOST}/api/user/profile`, {
      ...
      .then((response) => {
        ...
        setAvatar({
          preview: String(Http.RESOURCE_HOST + response.data.avatar),
        });
      })
      .catch((error) => {
        console.log(error);
  ...
  const onCloseDialog = () => {
    setAvatar({
      preview: String(Http.RESOURCE_HOST + user_default.avatar),
    });
  ...
  const onUpdateProfile = () => {
    ...
    const token = getLocal(FieldName.USER_TOKEN);
    axios
      .put(`${Http.HOST}/api/user/profile/update`, form_profile, {
        headers: {
          authorization: token,
      ...
      .then((res) => {
        setTimeout(() => {
          ...
          const full_name_element = document.getElementById(
            "full_name_current_user"
          );
          full_name_element.innerText = user.full_name;
          if (res.data.avatar) {
            saveLocal(FieldName.AVATAR, res.data.avatar);
            const avatar_element = document.getElementById(
              "avatar_current_user"
            );
            avatar_element.src = Http.RESOURCE_HOST + res.data.avatar;
          }
  ...
  const onUpdatePassword = () => {
    ...
    if (user_pass.new_password !== user_pass.confirm_password) {
      enqueueSnackbar("New password and confirm password not match!", {
        variant: "warning",
      });
      return;
    }
  ...
  return (
    <Dialog
      ...
      open={is_open}
      onClose={
        is_loading_update_profile || is_loading_update_pass
          ? null
          : onCloseDialog
      }
    >
      ...
      <DialogContent>
          ...
          <FormControl variant="outlined" size="small">
            ...
            <OutlinedInput
              ...
              onChange={(e) => {
                setUser({
                  ...user,
                  birthday: DateUtil.ConvertStringDateToDateForm(
                    e.target.value
                  ),
                });
              }}
        ...
        <Stack direction={"column"} style={{ marginTop: "2rem" }} spacing={3}>
          <FormControl
            ...
            variant="outlined"
          >
            <InputLabel htmlFor="input_curent_pass">
              Current password
            </InputLabel>
            <OutlinedInput
              ...
              onChange={(e) =>
                setUserPass({ ...user_pass, current_password: e.target.value })
              }
          ...
          >
            ...
            <OutlinedInput
              ...
              label="New password"
              onChange={(e) =>
                setUserPass({ ...user_pass, new_password: e.target.value })
              }
          ...
          <FormControl
            ...
            variant="outlined"
          >
            <InputLabel htmlFor="input_confirm_pass">
              Confirm password
            </InputLabel>
            <OutlinedInput
              ...
              onChange={(e) =>
                setUserPass({ ...user_pass, confirm_password: e.target.value })
              }
