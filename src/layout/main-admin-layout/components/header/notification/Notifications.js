...
export default function Notifications() {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${Http.HOST}/api/notification/admin`, {
      ...
      .catch((error) => {
        console.log(error);
        enqueueSnackbar("Something went wrong, get notifications failed", {
          variant: "error",
        });
      });
    ...
    const onReceiveNotification = (data) => {
      ...
      setNotifications((prevNotifications) =>
        [notification].concat(prevNotifications)
      );
  ...
  return (
    <div className="nav-item dropdown" onClick={onNotifyContainerClicked}>
      ...
      >
        <div
          className={clsx(
            "d-inline-block div-icon-dropdown",
        ...
        <Scrollbars style={{ minWidth: "10rem", height: 300 }} autoHide>
          {notifications.map((notification, index) => (
            <NotificationItem
              ...
              avatar={String(Http.RESOURCE_HOST + notification.avatar)}
